...
//decrypts message according to assignment
...
int main(int argc, char *argv[])
{
	...
	while (1){
		...
		if (children < 5){//makes sure only 5 children
			...
			if (pid == 0) //children
			{
				...
				//checks if valid connection (first two things recieved are checkers)
				...
				//Passes message and key to decrypt_message and decrypts it
				char *message_pointer = message;
				char *key_pointer = key;
				decrypt_message(message_pointer, key_pointer);
			...
			//exit(0);
				...
				if ((child = waitpid(-1, &childExitMethod, 0)) > 0)
					child += -1;
			}
		}
	}
